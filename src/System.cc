...
cv::Mat System::GetFrameDrawerImage() { return mpFrameDrawer->DrawFrame(); }
...
bool System::SavePCDBinary(std::string path)
{
  ...
  Verbose::PrintMess("Saving point cloud with " + to_string(atlas.size()) +
                       " points",
                     Verbose::VERBOSITY_NORMAL);
  ...
  oss << path << std::put_time(ptm, "%Y-%m-%d_%H-%M-%S") << ".pcd";

  try {
    pcl::io::savePCDFileBinary(oss.str(), cloud);
  } catch (const std::exception &e) {
    Verbose::PrintMess("Error saving point cloud: " + std::string(e.what()),
                       Verbose::VERBOSITY_NORMAL);
    return false;
  }

  Verbose::PrintMess("Saved " + std::to_string(cloud.points.size()) +
                       " points to " + oss.str(),
                     Verbose::VERBOSITY_NORMAL);
...
bool System::SavePCDASCII(std::string path)
{
  pcl::PointCloud<pcl::PointXYZ> cloud;
  cloud.height = 1;
  cloud.width = mpAtlas->GetAllMapPoints().size();
